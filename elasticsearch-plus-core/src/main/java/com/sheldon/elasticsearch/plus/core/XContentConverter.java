...
public class XContentConverter {
    ...
    public static <T extends AbstractBaseObject> XContentBuilder object2Builder(T doc) throws IOException, ClassInaccuracyException, InstantiationException, IllegalAccessException {
        ...
        builder.field(Constants.DYNAMIC, document.dynamicType().getCode());
    ...
    private static void parseXContentBuilder(XContentBuilder builder, List<MappingDefinition> fieldMappingDefinitions) throws IOException, ClassInaccuracyException, InstantiationException, IllegalAccessException {
        builder.startObject(Constants.PROPERTIES);
    ...
    private static void parseXContentBuilder(XContentBuilder builder, MappingDefinition fieldMappingDefinition) throws IOException, ClassInaccuracyException, InstantiationException, IllegalAccessException {
        builder.startObject(fieldMappingDefinition.getKey());
        FieldTypeEnum fieldType = fieldMappingDefinition.getMapping().getFieldType();
        builder.field(Constants.TYPE, fieldType.getType());
        String format = fieldMappingDefinition.getFormat();
        if (FieldTypeEnum.DATE.equals(fieldType)) {
            if (null == format || format.isEmpty())
                throw new ClassInaccuracyException(Constants.CLASS_INACCURACY_DATE_MUST_WITH_FORMAT);
            builder.field(Constants.FORMAT, format);
        }
    ...
    public static MappingDefinition getFieldMappingDefinition(Field field) throws ClassInaccuracyException, InstantiationException, IllegalAccessException {
        ...
        if (null == mappingEnum) {
            throw new ClassInaccuracyException(Constants.CLASS_INACCURACY_FIELD_MAPPING_NOT_SUPPORTED);
        }
        ...
        if (FieldTypeEnum.NESTED.equals(parentField.fieldType())) {
            ...
            if (mappingDefinition.isNotReady()) {
                throw new ClassInaccuracyException(Constants.CLASS_INACCURACY_NESTED_DATA_MUST_WITH_CHILDREN);
            }
        }
